...
import { IDimension, IPoint, SHAPES, getLeftBottomCircleCenter, getLeftTopCircleCenter, getRightBottomCircleCenter, getRightTopCircleCenter, selectionIndicateCircle, selectionIndicateRectangle } from "../Utils/Common";
...
export class Text extends Shape {
  ...
  drawOutline(ctx: CanvasRenderingContext2D): void {
    const position: IPoint = {posX: this.boundingBox.x - 5, posY: this.boundingBox.y + 14};
    const dimension: IDimension = {width: this.boundingBox.width + 10, height: this.boundingBox.height + 14};
    const leftTopCircleCenter:IPoint = getLeftTopCircleCenter(position);
    const leftBottomCircle:IPoint = getLeftBottomCircleCenter(position, dimension);
    const rightTopCircle: IPoint = getRightTopCircleCenter(position, dimension);
    const rightBottomCircle: IPoint = getRightBottomCircleCenter(position, dimension);
